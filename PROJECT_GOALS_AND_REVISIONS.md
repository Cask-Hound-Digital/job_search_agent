...
2. **{{PREVIOUS_COMPANY_1}} Date Range Requirement (June 2008 - August 2026)**:
...
3. **Silent Background Daemon Server Architecture**:
   ...
   * Launched silently using `subprocess.CREATE_NO_WINDOW` via `ensure_dashboard_server_running()` in [`fetch_gmail_alerts.py`](file:///c:/Users/mark\.gemini\antigravity\scratch\job-search-consultant\fetch_gmail_alerts.py) so no shell windows pop up or remain open on screen.

4. **Additive Queue Protection Rule**:
...
5. **Director-Level Role Scope Negotiation Strategy**:
...
6. **Open-Source Multi-Board JobSpy Integration**:
...
7. **Strict Location Filtering Engine (Remote & DFW Local Only)**:
...
8. **Authoritative Candidate Baseline Career Truth**:
...
* **{{PREVIOUS_COMPANY_1}} End Date Updated to August 2026**:
  * Updated candidate baseline in [`build_application_package.py`](file:///c:/Users/mark\.gemini\antigravity\scratch\job-search-consultant\build_application_package.py), [`current_payload.json`](file:///c:/Users/mark\.gemini\antigravity\scratch\job-search-consultant\current_payload.json), and [`state.json`](file:///c:/Users/mark\.gemini\antigravity\scratch\job-search-consultant\state.json) so {{PREVIOUS_COMPANY_1}} date range reads **June 2008 - August 2026** across all generated resumes and cover letters.
...
| **`fetch_gmail_alerts.py`** | [`./fetch_gmail_alerts.py`](file:///c:/Users/mark/.gemini/antigravity/scratch/job-search-consultant/fetch_gmail_alerts.py) | Email job alert parser with silent background server auto-launch & additive queue merging. |
| **`dashboard_server.py`** | [`./dashboard_server.py`](file:///c:/Users/mark/.gemini/antigravity/scratch/job-search-consultant/dashboard_server.py) | **[Local API Server]** Background HTTP server listening silently on localhost:5000 for 1-click apply triggers. |
...
| **`build_application_package.py`** | [`./build_application_package.py`](file:///c:/Users/mark/.gemini/antigravity/scratch/job-search-consultant/build_application_package.py) | **[Single Master Generator]** Unified Python engine configured with {{PREVIOUS_COMPANY_1}} end date August 2026. |
